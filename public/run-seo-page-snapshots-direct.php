<?php
// Purpose: Run the seo_page_snapshots migration and refresh directly from a public web endpoint without depending on artisan command registration. Date: 2026-03-29. Author: Mohsin.
...
if ($useToken) {
    if ($token === 'SET_A_TEMP_SECRET') {
        ...
        echo "Configure \$token inside public/run-seo-page-snapshots-direct.php before running.\n";
        exit;
    }
...
echo "SEO snapshot direct runner v3\n";
...
try {
    ...
    echo "Direct runner completed.\n";
    echo "Delete public/run-seo-page-snapshots-direct.php after one successful run.\n";
} catch (Throwable $throwable) {
    report($throwable);
    safeHttpResponseCode(500);
    echo "Direct runner failed: ".$throwable->getMessage()."\n";
}
