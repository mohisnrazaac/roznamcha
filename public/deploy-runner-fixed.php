...
ensureComposerEnvironment($rootPath);
...
function ensureComposerEnvironment(string $rootPath): void
{
    $home = getenv('HOME');
    ...
    $baseDir = $rootPath.'/storage/app/deploy-home';
    $composerDir = $baseDir.'/.composer';

    foreach ([$baseDir, $composerDir] as $dir) {
        if (! is_dir($dir)) {
            if (! mkdir($dir, 0775, true) && ! is_dir($dir)) {
                throw new RuntimeException("Unable to create directory at {$dir}");
            }
        }
    ...
    putenv('XDG_CACHE_HOME='.$baseDir.'/.cache');

    $_ENV['HOME'] = $baseDir;
    $_ENV['COMPOSER_HOME'] = $composerDir;
    $_ENV['XDG_CACHE_HOME'] = $baseDir.'/.cache';

    $_SERVER['HOME'] = $baseDir;
    $_SERVER['COMPOSER_HOME'] = $composerDir;
    $_SERVER['XDG_CACHE_HOME'] = $baseDir.'/.cache';
}
